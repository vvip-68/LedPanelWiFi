...
void lifeRoutine() {
  ...
  if (loadingFlag) {
    ...
    if (world == NULL)   { world   = new uint8_t*[pHEIGHT]; if (world != NULL) {for (uint8_t i = 0; i < pHEIGHT; i++) { world[i] = new uint8_t [BYTE_WIDTH]; }}}
    if (gener == NULL)   { gener   = new uint8_t*[pHEIGHT]; if (gener != NULL) {for (uint8_t i = 0; i < pHEIGHT; i++) { gener[i] = new uint8_t [BYTE_WIDTH]; }}}
    if (arr_crc == NULL) { arr_crc = new uint16_t[pHEIGHT]; }

    if (world == NULL || gener == NULL || arr_crc == NULL) {
  ...
  uint16_t crc = getCrc16((uint8_t*)arr_crc, pHEIGHT * 2);
