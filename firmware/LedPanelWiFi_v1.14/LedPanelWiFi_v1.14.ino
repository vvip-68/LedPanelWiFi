...
#define FIRMWARE_VER F("WiFiPanel v.1.14.2024.1209")

// К СВЕДЕНИЮ -------------------------------------------------------------------------------------------------
// 
// Время от времени после сборкт и загрузки прошивке контроллер могут наблюдаться следующие странные проявления:
//   - работат все эффекты, кроме некоторых. На некоторых эффектах нет вывода на матрицу, остается изрбраждение предыдущего эффекта и только мерцает первый светодиод на матрице
//   - эффекты работают стабильно, но при включении бегущей строки или календаря эффект замирает, только мерцает первый светодиод на матрице
//   - эффекты не отображается, мерцает первый светодиод на матрице, но при запуске бегущей строки, показе даты в часах, переключении настройки впрдк "Цветная температура" все отмирает
// Также на некоторых лентах и старых китайских матрицах типа 16x16, 32x8 выпуска до 2019-2020 годов могут наблюдаться следующие эффекты
//   - на матрице постоянно горит первый свтодиод разными цветами
//   - изображение на матрице (особенно часы, текст) - как будто сдвинуты строки, колонки так, что текст / цифры рассыпается и становится нечитаемым
//   - постоянные случайные всполохи и вспышки разными цветами в случайных местах
//   - яркость не регулируется - точки горят правктически на полной яркости
//   - вокруг основного изображения - как буд-то ореол из горящих соседних пикселей
//   - заданным цветам не соответствует - как правило какой-то случайный цвет
// При этом видно, что микроконтроллер управляет матрицей - эффекты двигаются, режимы переключаются, но целостной картины нет.
// При подключении того же самого микроконтроллера к более свежей матрице 2022+ годов - никаких проблем нет, все корректно отображается
//
// Обычно это происходит на контроллерах ESP8266. На ESP32 - редко.
// 
// Для лечения этого проявления выполните следующее: на примере FastLED 3.6.0, впрочем к другми тоже применимо
// Идете в папку где установлена библиотека FastLED. 
// Для ESP8266 папка C:\Users\<user>\Documents\Arduino\libraries\FastLED\src\platforms\esp\8266 и находите там файлы clockless_esp8266.h и, возможно clockless_block_esp8266.h
// Для ESP32 - папка C:\Users\<user>\Documents\Arduino\libraries\FastLED\src\platforms\esp\32 и файлы clockless_i2s_esp32.h и, возможно clockless_block_esp32.h 
// Где-то в первых строках этих файлов находите строку
// template <int DATA_PIN, int T1, int T2, int T3, EOrder RGB_ORDER = RGB, int XTRA0 = 0, bool FLIP = false, int WAIT_TIME = 85> 
// Обратите внимание в конце строки параметр "WAIT_TIME = 85". В FastLED 3.6.0 значение по умолчанию - 85 (для ESP32 - 5).
// Попробуйте увеличивать это значение и с каждым изменением пересобирать проект и смотреть - не исчезла ли проблема...
// У меня былим две подобные матрицы. На одной нормально заработало при значении 150, на другой - 350
// Для ESP32 замените дефолтное значение 5 на 10-25-50 - возможно при каком-то из них при пересборке проекта проблема исчезнет
//
// -------------------------------------------------------------------------------------------------------
//
// Внимание!!!
...
//
// Для ESP32 этот файл находится в папке C:\Users\<user>\Documents\Arduino\libraries\FastLED\src\platforms\esp\32
// Файл содержит строку *clocless* в названии. Для разных типов микроконтроллеров (в зависимости от выбранного в настройках) могут использоваться разные файлы
// Все файлы содержат похожую строку
//   template <int DATA_PIN, int T1, int T2, int T3, EOrder RGB_ORDER = RGB,int XTRA0 = 0, bool FLIP = false, int WAIT_TIME = 5>
// Значение по-умолчанию - 5. Попробуйте менять на значения "рядом" - 10,15,20.
// Однако будьте внимательны - при некоторых изменениях может сломаться даже то, что раньше работало - могут появиться всполохи на матрице
// и случайные вспышки даже там, где их раньше не было.
//
...
// библиотека использует конкурирующие процессы, обеспечивающие вывод дввнных на ленту и обработку стека WiFi.
// Стек WiFi - проигрывает, усьановить подключение к сети не удается.
