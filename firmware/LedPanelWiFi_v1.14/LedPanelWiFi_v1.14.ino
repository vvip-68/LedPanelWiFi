...
#define FIRMWARE_VER F("WiFiPanel v.1.14.2024.1104")
...
// 
// Лечение (на примере FastLED 3.6.0? впрочем к другми тоже применимо):
...
// Обратите внимание в конце строки параметр "WAIT_TIME = 85". В FastLED 3.6.0 значение по умолчанию - 85.
// В каких-то версиях больше, в каких-то меньше - не суть.
// Попробуйте увеличивать это значение и с каждым изменением пересобирать проект и смотреть - не улучштлось ли...
// У меня былим две подобные матрицы. На одной нормально заработало при значении 150, на другой - 350
// Каких-то проблем с отваливанием WiFi или еще с чем у меня не наблюдалось. И глюки, описанные выше - пропали.
// Если не получилось с увеличением - попробуйте уменьшать до 5 или 10. Однажды уменьшение до 10 помогло мне запустить
// гирлянду на тонкой проволчной основе. Однако на новых матрицах 2022-2024 года изменение значения 85 на 50  привело к тому, 
// что они стали работатьс теми же глюками, что описаны выше - артефакты, вспышки и т.д.
// Если ничего не помоглдо - ну, тогда выбросить матрицу...
...
// с последней на текущий момент версией 3.7.4 некоторые эффекты подергиваютчся (нет плавности движения)
//
// -------------------------------------------------------------------------------------------------------
//
// *** ArduinoJSON
