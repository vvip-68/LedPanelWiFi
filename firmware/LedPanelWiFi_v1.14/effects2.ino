...
void prizmata2Routine() {
  ...
  uint8_t effectBrightness = getBrightnessCalculated(globalBrightness, getEffectContrastValue(MC_PRIZMATA2));
  uint8_t sinbpm = map8(getEffectScaleParamValue(MC_PRIZMATA2),1,242);
  
  shiftDown();

  EVERY_N_SECONDS(15) { chooseNextPrizmataPalette(); }
...
void chooseNextPrizmataPalette() {
  gCurrentGradientPaletteNumber = random8(gGradientPrizmataPaletteCount);
  gTargetGradientPalette = gGradientPrizmataPalettes[ gCurrentGradientPaletteNumber ];
}
...
void shadows2Routine() {
  ...
  uint8_t effectBrightness = getBrightnessCalculated(globalBrightness, getEffectContrastValue(MC_SHADOWS2));
  uint8_t sinbpm = map8(getEffectScaleParamValue(MC_PRIZMATA2),1,242);
    
  shiftDown();

  EVERY_N_SECONDS(10) { chooseNextShadowsPalette(); }
  EVERY_N_MILLISECONDS(10) { nblendPaletteTowardPalette( gCurrentGradientPalette, gTargetGradientPalette, 12); }
