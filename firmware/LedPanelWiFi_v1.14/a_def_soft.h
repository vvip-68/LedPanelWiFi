...
#define EEPROM_OK     0xCE                       // Флаг, показывающий, что EEPROM инициализирована корректными данными 
...
#define DEFAULT_MQTT_USER   "user-name"          // Имя mqtt-пользователя    (!!! укажите имя пользователя для вашего соединения - зарегистрируйтесь на mqtt.by -> смотрите в личном кабинете !!!)
#endif

#ifndef DEFAULT_MQTT_PASS
#define DEFAULT_MQTT_PASS   "password"           // Пароль mqtt-пользователя (!!! укажите пароль вашего соединения - зарегистрируйтесь на mqtt.by -> смотрите в личном кабинете !!!)
#endif
...
#define TIME_ZONE_MIN    0                      // Актуально для некоторых стран индо-азиатского региона, у которых временная зона UTC 
#endif                                          // может быть +5:30 (часовой пояс Индии и Шри-Ланки) или даже +5:45 (часовой пояс Непала)
...
// Также требуется исправить список эффектов EFFECT_LIST (см. определение в файле a_def_lang.h) так, чтобы названия и порядок эффектов
...
// В EEPROM хранятся блоки описания параметров эффектов (с адреса 400 и далее 10 байт на эффект). 
...
#define EEPROM_MAX    1024                  // Максимальный размер EEPROM доступный для использования (4096)
#define EFFECT_EEPROM  400                  // Начальная ячейка eeprom с параметрами эффектов, 10 байт на эффект

// *************************************************************************

enum  eModes   {NORMAL, COLOR, TEXT}        parseMode; // Текущий режим парсера
enum  eSources {NONE, BOTH, UDP, MQTT, WEB} cmdSource; // Источник команды; NONE - нет значения; BOTH - любой, UDP-клиент, MQTT-клиент
...
#define  TEXT_STORAGE  F("/txt")            // Все строки хранятся в файловой системе микроконтроллера в папке '/txt'
...
#define  QSIZE_OUT 48                       // размер очереди исходящих сообщений Web/MQTT
#define  QSIZE_IN 16                        // размер очереди входящих команд от Web/MQTT

char     cmdQueueType[QSIZE_IN];            // кольцевой буфер очереди полученных команд от Web/MQTT - источник команды 'M' - MQTT, 'W' - Web Socket
String   cmdQueue[QSIZE_IN];                // кольцевой буфер очереди полученных команд от Web/MQTT - команда
...
String   tpcWQueue[QSIZE_OUT];              // Кольцевой буфер очереди отправки команд в Web (topic)
String   outWQueue[QSIZE_OUT];              // Кольцевой буфер очереди отправки команд в Web (message)
uint8_t  outWQueueWriteIdx = 0;             // позиция записи в очередь отправки MQTT сообщений
uint8_t  outWQueueReadIdx = 0;              // позиция чтения из очереди отправки MQTT сообщений
uint8_t  outWQueueLength = 0;               // количество команд в очереди отправки MQTT сообщений
uint32_t lastWebSend = 0;                   // время последней отправки сообщения в web-канал
...
int8_t   timeZoneOffset = TIME_ZONE;            // смещение часового пояса от UTC - часы
int8_t   timeZoneOffsetMinutes = TIME_ZONE_MIN; // смещение часового пояса от UTC - минуты
...
bool     not_check_time = true;             // Отключить защиту от резкого изменения времени при получении с NTP
...
#define    BUF_CMD_SIZE  512                // максимальный размер выделяемого буфера для входящих сообщений по Web/MQTT каналу
char       incomeCmdBuffer[BUF_CMD_SIZE];   // Буфер для приема строки команды из Web/MQTT
...
int16_t    packetSize = 0;                 // Размер принятого пакета
...
uint8_t    mapListLen = 0;
  ...
  uint32_t OVERLAY_SIZE  =  pWIDTH * 21;         // высота шрифта 16 + 3 строки диакритич символов над знакоместом и две - под знакоместом
#else
  // Шрифт размером 8x13
  uint32_t OVERLAY_SIZE =   pWIDTH * 18;         // высота шрифта 13 + 3 строки диакритич символов над знакоместом и две - под знакоместом
#endif
