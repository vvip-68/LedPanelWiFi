...
enum  eSources {NONE, BOTH, UDP, WEB}       cmdSource; // Источник команды; NONE - нет значения; BOTH - любой, UDP-клиент, WEB-клиент
...
String   preparedTextStorage;               // Хранилище строки, которая была считана последним вызовом getTextByIdx(); - "FS' или "SD"
...
#if defined(ESP32)
  #define  QSIZE_OUT 32                     // размер очереди исходящих сообщений Web
  #define  QSIZE_IN 8                       // размер очереди входящих команд от Web
#else
  #define  QSIZE_OUT 64                     // размер очереди исходящих сообщений Web
  #define  QSIZE_IN 12                      // размер очереди входящих команд от Web
#endif
...
uint8_t  outWQueueWriteIdx = 0;             // позиция записи в очередь отправки Web сообщений
uint8_t  outWQueueReadIdx = 0;              // позиция чтения из очереди отправки Web сообщений
uint8_t  outWQueueLength = 0;               // количество команд в очереди отправки Web сообщений
...
bool     isAuxActive = false;               // Линия дополнительного управления питанием активна
...
char*      incomeBuffer;                    // Буфер для приема строки команды из wifi udp сокета; также используется для загрузки строк из EEPROM
...
String     system_name;                          // Имя для отображения в WebUI
...
static const char sTIME[]    PROGMEM = "TIME";
