...
// В некоторых случаях непонятного происхождения отсутствует вывод на матрицу - мерцает только самый первый светодиод матрицы
// Это может происходить как на всех эффектах, так и только на одном определенном, например "Мерцание"ю
// При этом если в Web-UI запустить показ бегущей строки - вывод на матрицу на это время восстанавливается
// Если такое поведение обнаруживается - пробуем собрать прошивку с MAGIC_1 = 1, если не помогло - ставим также MAGIF_2 = 0
// Если не помогло - читаем комментарий в основном файле, раздел "Настройки Arduino IDE"
...
#define DEVICE_VARIANT 0      /* Поставьте 0 - если ваше устройство использует только часть из доступных компонентов в схемах с ESP8266 - не полная схема */
                              ...
                              //   для DEVICE_VARIANT == 0 --> строки 135-257
                              //   для DEVICE_VARIANT == 1 --> строки 259-372
#endif
...
//   Б) ПЕРЕД ЗАГРУЗКОЙ ПРОШИВКИ В МИКРОКОНТРОЛЛЕР ИЗМЕНИТЕ ЗНАЧЕНИЕ КОНСТАНТЫ EEPROM_OK В ФАЙЛЕ a_def_soft.h НА любое ДРУГОЕ шестнадцатеричное значение
...
#define SD_CS_PIN SS            // !!! не менять !!! - пин выбора SD карты - D8 (SS); Также SD карта использует D5 (SCLK), D6 (MISO), D7 (MOSI) (пины определены в ядре микрооконтроллера - аппаратный SPI)
...
#define AUX_PIN D0              /* D0 управляющий пин вкл/выкл доп.оборудование режимы по времени через MOSFET / реле; AUX_ON - HIGH, AUX_OFF - LOW */
...
#define SD_CS_PIN SS            // !!! не менять !!! - пин выбора SD карты - D8 (CS); Также SD карта использует D5 (CLK), D6 (MISO), D7 (MOSI) (пины определены в ядре микрооконтроллера - аппаратный SPI)
