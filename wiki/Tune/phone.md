...
Введите в адресной строке браузера IP-адрес, который вы назначили устройству на предыдущем шаге. Дождитесь загрузки страницы.
Впрочем, теперь, вероятнее всего вам будет удобнее открыть браузер по этому адресе на компьютере.  

Web-приложение в браузере на телефоне и в браузере на компьютере обладают абсолютно одинаковым функционалом,
...
На странице настроек собраны элементы управления, позволяющие установить следующие параметры устройства:  
...
- **Сеть** - настройка подключения устройства к локальной сети, параметры работы устройства в режиме точки доступа  
...
Выберите вариант **"Карта индексов"** ***(1)***. Как подготовить карту индексов описано в этой [статье](https://github.com/vvip-68/LedPanelWiFi/wiki/Сборная-матрица,-состоящая-из-матриц-разного-типа-и-размера). 
...
Если карта индекса сохранена в файл и загружена в файловую систему микроконтроллера - комбобокс будет содержать все найденный при запуске микроконтроллера
...
- **Источник (мастер)** - устройство является ведущим, изображение с его матрицы будет транслироваться на другие устройства группы.  
- **Приемник (ведомый)** - устройство будет получать изображение на матрицу с другого (ведущего) устройства в группе.  

![Группа-2](https://github.com/vvip-68/LedPanelWiFi/blob/main/wiki/Tune/p12-4-2.png)

В комбобоксе **"Тип синхронизации"** ***(2)*** укажите в каком порядке данные будут приходить к *ведомому* устройстве или 
в каком порядке *ведущее* устройство будет отправлять данные в сеть:  
...
Завершите настройку режимов сихронизации нажав на кнопку **"Применить"** ***(4)***.  
...
В поле **"IP адрес"** ***(5)*** укажите какой ***IP адрес*** должно получить ваше устройство при подключении к сети.  
...
Параметр **Сервер погоды** ***(1)*** определяет с какого погодного сервера устройство будет запрашивать информацию Доступные значения:  
...
- Значение, указанное в поле **"Лимит по току"** позволяет программным образом ограничить максимальную яркость светодиодов и уберечь блок питания от срабатывания защиты
  ...
  которое требуется отводить от ленты. При недостаточном отводе тепла диоды будут перегреваться, что сократит срок их службы или вовсе выведет из строя.  
...
если файл сохраненных настроек найден *в файловой системе* или **SD**, если файл сохраненных настроек найден *на SD-карте*.  
...
Если по какой-то причине устройство требуется перезагрузить, физический доступ к устройству затруднен, но устройство доступно через Web-интерфейс - 
нажмите кнопку **"Перезагрузить"**. Устройство перезагрузится. На время перезагрузки контроллера соединение с устройством будет потеряно, но затем через некоторое время должно восстановиться.
